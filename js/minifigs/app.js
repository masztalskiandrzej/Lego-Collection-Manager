...
const App = {
    ...
    async handleAutoFillData() {
        ...
        if (!num) {
            Dialogs.alert(t('msg.enterMinifigNumber'), { type: 'warning' });
        ...
        try {
            ...
            const result = await lookup({ itemNumber: num, itemType: 'minifigure' });
            const data = result && result.data;

            if (!data || !data.name) {
                Dialogs.alert(t('msg.notFoundMinifigShort', { n: num }), { type: 'warning' });
                return;
            }
            ...
            // minifigurki nie mają pola liczby elementów
            if (data.imageUrl) {
    ...
    extractScannedNumber(text) {
        ...
        if (urlMatch) return urlMatch[1];
                // Numery figurek: sw0999, col001, fig-002544 lub zwykły 4-7-cyfrowy
        const figMatch = s.match(/\b((?:sw|col)[-_]?\d{3,6}|fig[-_]?\d{4,6})\b/i);
